...
def _directional_driver_counts(row: Dict[str, Any], obs_agg: Optional[Dict[str, Any]] = None) -> Dict[str, int]:
    """
    Translate AI transmission direction into generic positive/negative
    driver counts for the affected ticker.

    Important:
    - ai_transmission_scores is intentionally kept aligned with the legacy
      table schema, so observation_count is not written into legacy rows.
    - For generic structural_theme_scores, observation counts are recovered
      from obs_aggs using map_id.
    ...
    if obs_agg is not None:
        obs_count = int(obs_agg.get("observation_count") or 0)
    else:
...
def build_structural_theme_score_rows(
    ...
    for ticker, ticker_rows in by_ticker.items():
        ...
        # Recover evidence counts from observation aggregates using map_id.
        # Do not add observation_count to score_rows because those rows are
        # directly upserted into the legacy ai_transmission_scores table.
        ...
        for r in ticker_rows:
            ...
            obs_agg = obs_aggs.get(map_id_int) if map_id_int is not None else None
            obs_count = int(obs_agg.get("observation_count") or 0) if obs_agg else 0
            total_observations += obs_count

            counts = _directional_driver_counts(r, obs_agg)
        ...
        for r in ranked:
            map_id_for_relationship = r.get("map_id")
            try:
                map_id_for_relationship_int = int(map_id_for_relationship)
            except Exception:
                map_id_for_relationship_int = None

            obs_agg_for_relationship = (
                obs_aggs.get(map_id_for_relationship_int)
                if map_id_for_relationship_int is not None
                else None
            ) or {}

            relationship = {
                "map_id": r.get("map_id"),
                "observation_count": int(obs_agg_for_relationship.get("observation_count") or 0),
                "observation_aggregate": obs_agg_for_relationship,
        ...
        structural_rows.append(
            {
                ...
                "metadata": {
                    "source_script": "ai_transmission_scoring_v2_phase1_refactor.py",
...
def write_structural_theme_scores(
    ...
    structural_rows = build_structural_theme_score_rows(score_rows, obs_aggs, run_date_sgt)
...
def main() -> int:
    ...
    try:
        ...
        structural_rows_written = write_structural_theme_scores(score_rows, obs_aggs, run_date)
