...
CREATE TABLE IF NOT EXISTS public.tier3h5_institutional_issuer_registry (
  ...
  CONSTRAINT tier3h5_issuer_name_key UNIQUE (issuer_name_normalized),
...
CREATE TABLE IF NOT EXISTS public.tier3h5_institutional_security_registry (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  CONSTRAINT tier3h5_security_deterministic_key UNIQUE (exchange, ticker, security_type, source_registry)
);
...
CREATE INDEX IF NOT EXISTS idx_tier3h5_security_issuer_id
  ON public.tier3h5_institutional_security_registry(issuer_id);
CREATE INDEX IF NOT EXISTS idx_tier3h5_provenance_ingestion_run_id
  ON public.tier3h5_registry_provenance(ingestion_run_id);
