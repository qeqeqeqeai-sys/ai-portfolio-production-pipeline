...
def main() -> int:
    parser = argparse.ArgumentParser(description="OBS-QUERY-3 deterministic historical-vs-live DB-2 fact comparison retrieval.")
    parser.add_argument("--comparison-type", required=True, choices=COMPARISON_TYPES)
    parser.add_argument("--historical-source-layer", help="Historical source layer / phase_id filter.")
    parser.add_argument("--live-source-layer", help="Live source layer / phase_id filter.")
    ...
    parser.add_argument("--limit", type=int, default=DEFAULT_LIMIT, help="Bounded result limit; hard maximum is enforced by the retrieval module.")
    ...
    result = compare_historical_live_state(
        comparison_type=args.comparison_type,
        client=client,
        fact_rows=fact_rows,
        historical_source_layer=args.historical_source_layer,
        live_source_layer=args.live_source_layer,
        symbol=args.symbol,
    ...
    if client is None and not fact_rows:
        result["runtime_warnings"] = [
            "SUPABASE_URL and Supabase key env var were not both present; emitted deterministic empty OBS-QUERY-3 output without external provider calls."
        ]
    paths = write_historical_live_comparison_outputs(result, output_json=args.output_json, output_md=args.output_md)
    print(json.dumps({"status": "ok", "query_type": result["query_type"], "comparison_type": result["comparison_type"], "result_count": result["result_count"], "effective_limit": result["effective_limit"], **paths}, sort_keys=True))
    return 0
