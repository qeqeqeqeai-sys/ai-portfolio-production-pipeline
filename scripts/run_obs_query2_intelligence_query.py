...
def main() -> int:
    parser = argparse.ArgumentParser(description="OBS-QUERY-2 bounded DB-2 intelligence question retrieval.")
    ...
    result = retrieve_intelligence_question(
    ...
    if client is None and not fact_rows:
        result["runtime_warnings"] = [
            "SUPABASE_URL and Supabase key env var were not both present; emitted deterministic empty OBS-QUERY-2 output without external provider calls."
        ]
    paths = write_intelligence_question_outputs(result, output_json=args.output_json, output_md=args.output_md)
    print(json.dumps({"status": "ok", "query_type": result["query_type"], "result_count": result["result_count"], "effective_limit": result["effective_limit"], **paths}, sort_keys=True))
    return 0
