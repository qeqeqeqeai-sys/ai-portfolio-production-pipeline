...
Run
---
python ai_portfolio_engine_v7_alpha_score_github_actions.py
"""
...
load_dotenv()  # Local only. GitHub Actions uses repository secrets/environment variables.
...
SOURCE = "GITHUB_ACTIONS_PORTFOLIO_ENGINE_V7_ALPHA_SCORE"
...
def log(message: str, level: str = "INFO") -> None:
    ts = dt.datetime.now(SGT).strftime("%Y-%m-%d %H:%M:%S")
    print(f"[{ts} SGT] [{level}] {message}", flush=True)


def save_artifacts(portfolio: pd.DataFrame, rows: List[Dict[str, Any]]) -> None:
    rows_df = pd.DataFrame(rows)
    rows_path = OUTPUT_DIR / f"portfolio_engine_v7_rows_{RUN_DATE_SGT}.csv"
    rows_df.to_csv(rows_path, index=False)
    log(f"Saved portfolio rows artifact: {rows_path}")

    if not portfolio.empty:
        holdings_path = OUTPUT_DIR / f"portfolio_engine_v7_holdings_{RUN_DATE_SGT}.csv"
        portfolio.to_csv(holdings_path, index=False)
        log(f"Saved portfolio holdings artifact: {holdings_path}")

    diagnostics_path = OUTPUT_DIR / f"portfolio_engine_v7_summary_{RUN_DATE_SGT}.json"
    summary = {
        "run_date_sgt": RUN_DATE_SGT,
        "equity_holdings": int(len(portfolio)),
        "rows_written": int(len(rows)),
        "total_weight": round(float(sum(float(r.get("portfolio_weight", 0) or 0) for r in rows)), 6),
        "cash_weight": round(float(sum(float(r.get("portfolio_weight", 0) or 0) for r in rows if r.get("ticker") == "CASH")), 6),
        "source": SOURCE,
    }
    diagnostics_path.write_text(json.dumps(summary, indent=2), encoding="utf-8")
    log(f"Saved portfolio summary artifact: {diagnostics_path}")
...
def sb_upsert(table: str, rows: List[Dict[str, Any]], conflict_cols: str, batch_size: int = 500) -> None:
    ...
    for i in range(0, len(rows), batch_size):
        ...
        log(f"Upserted rows {i + 1} to {i + len(batch)} / {len(rows)} into {table}")
...
def load_latest_v7_scores() -> pd.DataFrame:
    ...
    if df.empty:
        log(f"No v7 scores found for RUN_DATE_SGT={RUN_DATE_SGT}. Searching latest available date.", "WARN")
        ...
        log(f"Using latest available v7 score date: {latest_date}")
    ...
    log(f"Loaded v7 alpha scores: {len(df)} rows")
    log(f"Score date used: {df['run_date_sgt'].iloc[0]}")
...
def validate_portfolio_rows(rows: List[Dict[str, Any]]) -> None:
    if not rows:
        raise RuntimeError("Portfolio engine produced zero output rows.")

    total_weight = sum(float(r.get("portfolio_weight", 0) or 0) for r in rows)
    if abs(total_weight - 1.0) > 0.01:
        raise RuntimeError(f"Portfolio weights do not sum close to 1.0. Total={total_weight:.6f}")

    tickers = [r.get("ticker") for r in rows]
    if len(tickers) != len(set(tickers)):
        raise RuntimeError("Duplicate tickers detected in portfolio output rows.")

    log(f"Validation passed. Output rows={len(rows)}, total_weight={total_weight:.6f}")
...
def main() -> None:
    log("Starting portfolio engine v7 alpha score pipeline")
    log(f"RUN_DATE_SGT={RUN_DATE_SGT}")
    require_env()
    ...
    validate_portfolio_rows(rows)
    print_diagnostics(df, weighted, rows)
    save_artifacts(weighted, rows)

    sb_upsert(TABLE_TARGET, rows, "run_date_sgt,ticker")

    log(f"Portfolio engine v7 complete. Rows written to {TABLE_TARGET}: {len(rows)}", "DONE")
