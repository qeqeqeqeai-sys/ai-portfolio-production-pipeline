...
def main() -> None:
    ...
    p.add_argument("--expected-chunk-count", type=int, default=5)
    ...
    args = p.parse_args()
    payload = run_hist_density3(trading_days=args.trading_days, max_symbols=args.max_symbols, symbol_chunk_size=args.symbol_chunk_size, expected_chunk_count=args.expected_chunk_count, raw_cache_enabled=args.raw_cache_enabled, raw_cache_write_enabled=args.raw_cache_write_enabled, cache_validation_mode=args.cache_validation_mode, cache_only_validation=args.cache_only_validation, include_high_risk_symbols=args.include_high_risk_symbols, apply_sde2_replacements=args.apply_sde2_replacements, dry_run_config_only=args.dry_run_config_only, output_root=args.output_root, density_mode=DENSITY_MODE_REAL)
