...
SOURCE = "PYTHON_ALPHA_V7_EFFICIENCY_ENABLED"
...
def sb_get(table: str, params: Dict[str, str], limit: int = 10000) -> pd.DataFrame:
    ...
    while True:
        ...
        if r.status_code == 404:
            print(f"[WARN] Table not found or unavailable: {table}")
            return pd.DataFrame()

        if r.status_code >= 400:
            print(f"[WARN] Could not read {table}: {r.status_code} {r.text[:300]}")
            return pd.DataFrame()
...
def load_universe() -> pd.DataFrame:
    ...
    print(f"[OK] Loaded active universe: {len(df)} stocks")
    return df
...
def load_latest_returns() -> pd.DataFrame:
    ...
    if df.empty:
        print("[WARN] ai_stock_returns unavailable or empty.")
        return pd.DataFrame()
    ...
    print(f"[OK] Loaded latest returns rows: {len(df)}")
    return df
...
def load_latest_v6() -> pd.DataFrame:
    ...
    if df.empty:
        print("[WARN] ai_stock_signal_scores_v6 unavailable or empty.")
        return pd.DataFrame()

    df["ticker"] = df["ticker"].apply(clean_ticker)
    df = latest_by_date(df, ["run_date_sgt", "created_at"])

    print(f"[OK] Loaded latest v6 signal rows: {len(df)}")
    return df
...
def load_latest_hype() -> pd.DataFrame:
    ...
    if df.empty:
        print("[WARN] ai_hype_scores unavailable or empty.")
        return pd.DataFrame()

    df["ticker"] = df["ticker"].apply(clean_ticker)
    df = latest_by_date(df, ["run_date", "run_date_sgt", "created_at"])

    print(f"[OK] Loaded latest hype rows: {len(df)}")
    return df
...
def load_optional_tables() -> Dict[str, pd.DataFrame]:
    ...
    for table in OPTIONAL_TABLES:
        ...
        if not df.empty and "ticker" in df.columns:
            ...
            print(f"[OK] Loaded optional table {table}: {len(df)} rows")
            out[table] = df
        else:
            print(f"[INFO] Optional table skipped: {table}")
    return out
...
def main() -> None:
    ...
    print(f"[DONE] v7 alpha scoring complete. Rows written to {TABLE_TARGET}: {len(rows)}")
