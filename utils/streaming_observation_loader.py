...
- Additive-only
- Idempotent and restart-safe when used with chunk checkpoints
...
# ---------------------------------------------------------------------
# Ensure repo root is importable when called from nested scripts
# ---------------------------------------------------------------------
...
class StreamingObservationLoader:
    """
    Source-table aware loader that uses the generic paginated REST loader.

    This keeps source-specific normalization outside the pagination utility.
    """
    ...
    def stream_observations(
        ...
        *,
        ...
        filters: Dict[str, str] = {
            ...
            f"{cfg.date_col}__lte_filter": f"lte.{end_date.isoformat()}",
        }

        # PostgREST cannot accept fake filter keys. Convert duplicate date filters
        # to the exact syntax by using normal query params through a compact trick:
        # the pagination loader accepts a dict, so we add the real lte key after.
        filters = {
            cfg.date_col: f"gte.{start_date.isoformat()}",
        }
        filters[f"{cfg.date_col}"] = f"gte.{start_date.isoformat()}"

        # For two predicates on the same column, use the native "and" expression.
        filters = {
    ...
    def load_grouped_observations_for_window(
        ...
        *,
        ...
        """
        Chunk-safe grouped observation load.

        This still materializes only the rolling reconstruction window for the
        current chunk, not a full source table or full history.
        """
