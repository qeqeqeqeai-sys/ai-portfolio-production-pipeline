...
def compute_replay_metrics(comparison: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "replay_difference_count": diff_count,
        "replay_structural_difference_count": struct,
        "replay_metadata_difference_count": meta,
        "replay_normalization_difference_count": norm,
        "replay_provenance_difference_count": prov,
        "replay_unresolved_difference_count": unresolved,
        "replay_consistency_ratio": ratio,
        "replay_exact_match": diff_count == 0,
        "normalization_replay_stable": norm == 0,
        "provenance_replay_stable": prov == 0,
        "governance_replay_stable": struct == 0 and norm == 0 and prov == 0,
    }
...
def run_phase2c_replay_governance(current_rows: list[dict[str, Any]] | None = None, prior_rows: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    status = classify_replay_governance(combined, baseline_available=comparison.get("baseline_available", False))

    drift_counter = Counter([status])
    replay_timestamp = datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z")

    lineage = {
        "phase": PHASE,
        "status": comparison.get("status"),
        "prior_registry_snapshot_id": prior[0].get("registry_snapshot_id") if prior else None,
        "compared_registry_snapshot_id": current[0].get("registry_snapshot_id") if current else None,
        "replay_comparison_run_id": f"replay-{(current[0].get('ingestion_run_id') if current else 'none')}",
        "replay_governance_status": status,
    ...
    drift_summary = {"phase": PHASE, "status": comparison.get("status"), "replay_governance_status": status, "drift_status_counts": dict(drift_counter), "source_additions": comparison.get("source_additions", []), "source_removals": comparison.get("source_removals", []), "difference_rows": comparison.get("differences", []), "enforcement_enabled": False, "canonical_override_enabled": False, "replay_mode": "advisory_only"}
    governance_summary = {"phase": PHASE, "status": comparison.get("status"), "replay_governance_status": status, **metrics, "enforcement_enabled": False, "canonical_override_enabled": False, "replay_mode": "advisory_only"}
    phase_summary = {"phase": PHASE, "status": comparison.get("status"), "replay_governance_status": status, "replay_exact_match": metrics["replay_exact_match"], "replay_consistency_ratio": metrics["replay_consistency_ratio"], "replay_difference_count": metrics["replay_difference_count"], "drift_status_counts": dict(drift_counter), "normalization_replay_stable": metrics["normalization_replay_stable"], "provenance_replay_stable": metrics["provenance_replay_stable"], "governance_replay_stable": metrics["governance_replay_stable"], "enforcement_enabled": False, "canonical_override_enabled": False, "replay_mode": "advisory_only"}
