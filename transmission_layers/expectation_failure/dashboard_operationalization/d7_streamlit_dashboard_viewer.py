...
def build_d7_dashboard_view_model(*, findings_payload: Mapping[str, Any], narratives_payload: Mapping[str, Any], evidence_payload: Mapping[str, Any], integrity_payload: Mapping[str, Any], historical_runs_payloads: list[Mapping[str, Any]] | None = None) -> OrderedDict[str, Any]:
    ...
    h3_inventory = build_h3_replay_transition_inventory(replay_windows=effective_history)
    h3_chains = build_h3_structural_transition_chains(transition_inventory=h3_inventory)
    h3_novelty = build_h3_transition_novelty_analysis(transition_inventory=h3_inventory, transition_chains=h3_chains)
    h3_risk = build_h3_transition_risk_diagnostics(transition_inventory=h3_inventory, novelty_analysis=h3_novelty)
    h3_recs = build_h3_operator_transition_recommendations(novelty_analysis=h3_novelty, risk_diagnostics=h3_risk)
    h3_dashboard = build_h3_dashboard_payload(transition_inventory=h3_inventory, transition_chains=h3_chains, novelty_analysis=h3_novelty, risk_diagnostics=h3_risk, operator_recommendations=h3_recs)
    h3_certification = certify_h3_cross_replay_structural_transition_intelligence(transition_inventory=h3_inventory, transition_chains=h3_chains, dashboard_payload=h3_dashboard)
    cd2_pool = build_cd2_replay_candidate_pool(replay_windows=effective_history, cd1_dashboard_payload=cd1_dashboard, h3_dashboard_payload=h3_dashboard)
    cd2_scorecard = build_cd2_novelty_scorecard(candidate_pool=cd2_pool)
    cd2_buckets = build_cd2_candidate_priority_buckets(candidate_pool=cd2_pool, novelty_scorecard=cd2_scorecard)
    cd2_rationale = build_cd2_replay_selection_rationale(candidate_pool=cd2_pool, novelty_scorecard=cd2_scorecard, priority_buckets=cd2_buckets)
    cd2_summary = build_cd2_prioritization_summary(candidate_pool=cd2_pool, novelty_scorecard=cd2_scorecard, priority_buckets=cd2_buckets)
    cd2_guardrails = build_cd2_operator_guardrails()
    cd2_dashboard = build_cd2_dashboard_payload(candidate_pool=cd2_pool, novelty_scorecard=cd2_scorecard, priority_buckets=cd2_buckets, selection_rationale=cd2_rationale, prioritization_summary=cd2_summary, operator_guardrails=cd2_guardrails)
    cd2_certification = certify_cd2_replay_novelty_prioritization(candidate_pool=cd2_pool, novelty_scorecard=cd2_scorecard, dashboard_payload=cd2_dashboard, operator_guardrails=cd2_guardrails)
    cd3_candidate_set = build_cd3_replay_expansion_candidate_set(cd2_candidate_pool=cd2_pool, cd2_novelty_scorecard=cd2_scorecard, cd2_priority_buckets=cd2_buckets)
    cd3_expansion_plan = build_cd3_bounded_expansion_plan(candidate_set=cd3_candidate_set)
    cd3_review_queue = build_cd3_operator_review_queue(expansion_plan=cd3_expansion_plan)
    ...
    payload = OrderedDict([
        ...
        ("h3_cross_replay_structural_transition_intelligence", h3_dashboard),
        ("h3_cross_replay_structural_transition_intelligence_certification", h3_certification),
        ("cd2_replay_novelty_prioritization", cd2_dashboard),
...
def render_h3_cross_replay_structural_transition_intelligence(view_model: Mapping[str, Any], *, st: Any) -> None:
    payload = view_model.get("h3_cross_replay_structural_transition_intelligence") if isinstance(view_model, Mapping) else {}
    st.subheader("H3 Cross-Replay Structural Transition Intelligence")
    for key in ("Transition Novelty Analysis", "Structural Transition Chains", "Transition Risk Diagnostics", "Operator Replay-Diversification Guidance", "Cross-Replay Transition Overview", "Contradiction Evolution", "Continuity-State Transitions", "Confidence-State Transitions", "Regime Transitions", "Semantic Theme Evolution"):
        st.markdown(f"**{key}**")
        st.json(payload.get(key, {}))
    with st.expander("H3 Governance/Lineage Controls"):
        st.json(payload.get("Governance/Lineage Controls", {}))


def render_cd2_replay_novelty_prioritization(view_model: Mapping[str, Any], *, st: Any) -> None:
    payload = view_model.get("cd2_replay_novelty_prioritization") if isinstance(view_model, Mapping) else {}
    st.subheader("CD2 Replay Novelty Prioritization")
    st.markdown("**Replay Novelty Prioritization Overview**")
    st.json(payload.get("Replay Novelty Prioritization Overview", {}))
    st.markdown("**Candidate Pool Summary**")
    st.json(payload.get("Candidate Pool Summary", [])[:5])
    st.markdown("**Novelty Scorecard**")
    st.json(payload.get("Novelty Scorecard", [])[:8])
    st.markdown("**Priority Buckets**")
    st.json(payload.get("Priority Buckets", {}))
    st.markdown("**Selection Rationale**")
    st.json(payload.get("Selection Rationale", [])[:6])
    st.markdown("**Diversification Summary**")
    st.json(payload.get("Diversification Summary", {}))
    st.markdown("**Saturation/Repetition Warnings**")
    st.json(payload.get("Saturation/Repetition Warnings", {}))
    st.markdown("**Recommended Operator Review Queue**")
    st.json(payload.get("Recommended Operator Review Queue", []))
    with st.expander("Governance/Operator Guardrails"):
        st.json(payload.get("Governance/Operator Guardrails", {}))


def render_cd3_governed_novelty_guided_replay_expansion_plan(view_model: Mapping[str, Any], *, st: Any) -> None:
    payload = view_model.get("cd3_governed_novelty_guided_replay_expansion_plan") if isinstance(view_model, Mapping) else {}
    st.subheader("CD3 — Governed Novelty-Guided Replay Expansion Plan")
    st.caption("Plan-only, deterministic, recommendation-only layer. No replay execution approval.")
    st.markdown("### Governed Replay Expansion Plan Overview")
    st.json(payload.get("Governed Replay Expansion Plan Overview", {}))
    st.markdown("### Selected Replay Candidates")
    st.json(payload.get("Selected Replay Candidates", []))
    st.markdown("### Operator Review Queue")
    st.json(payload.get("Operator Review Queue", []))
    st.markdown("### Deferred Replay Candidates")
    st.json(payload.get("Deferred Replay Candidates", []))
    st.markdown("### Novelty Coverage Summary")
    st.json(payload.get("Novelty Coverage Summary", {}))
    st.markdown("### Diversity Balance Summary")
    st.json(payload.get("Diversity Balance Summary", {}))
    st.markdown("### Concentration Risk Reduction Summary")
    st.write(payload.get("Concentration Risk Reduction Summary"))
    with st.expander("Governance Preflight Checklist"):
        st.json(payload.get("Governance Preflight Checklist", {}))
    st.markdown("### Expansion Plan Rationale")
    st.json(payload.get("Expansion Plan Rationale", {}))
...
def render_ix4_interpretability_hardening(view_model: Mapping[str, Any], *, st: Any) -> None:
    payload = view_model.get("ix4_interpretability_hardening") if isinstance(view_model, Mapping) else {}
    if not isinstance(payload, Mapping) or not payload:
        return
    st.markdown("### IX4 — Interpretability Hardening & Narrative Explainability")
    st.markdown(str(payload.get("Interpretability Hardening Overview", "")))
    for k in ("Cluster Explainability Cards","Narrative Explainability Analysis","Interpretability Scorecard","Narrative Boundary Enforcement","Operator Explainability Summary","Auditability Preview"):
        st.markdown(f"#### {k}")
        st.json(payload.get(k))
    with st.expander("Governance/Boundary Constraints"):
        st.json(payload.get("Governance/Boundary Constraints", {}))
