...
def replay_structural_timeline(snapshots: Iterable[Dict[str, Any]]) -> Dict[str, Any]:
    ordered = sorted([load_structural_snapshot(s) for s in snapshots], key=lambda s: (str(s.get("run_date_sgt", "")), str(s.get("simulation_run_id", "")), str(s.get("topology_hash", ""))))
    transitions = []
    ...
    topology_hash_sequence = [str(s.get("topology_hash", "")) for s in ordered]
    ...
    return normalize_deterministic({
        ...
        "recurring_cascade_diagnostics": {
            ...
            "repeated_propagation_bottlenecks": recurring_chokepoints,
            "repeated_cascade_corridors": recurring_failed,
        },
