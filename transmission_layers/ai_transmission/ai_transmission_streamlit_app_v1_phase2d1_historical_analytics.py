...
def fetch_phase2d_history_table(
    ...
    - tries run_date_sgt first
    - falls back to run_date
    - finally falls back to unrestricted fetch and filters locally
    """
    if not SUPABASE_URL or not SUPABASE_ANON_KEY:
        return pd.DataFrame()
    ...
    attempts = []
    for date_col in ["run_date_sgt", "run_date"]:
        params = {
            "select": "*",
            "theme_name": f"eq.{theme_name}",
        ...
        if start_date:
            params[date_col] = f"gte.{start_date}"
        if end_date:
            params[date_col] = f"lte.{end_date}"
        attempts.append(params)

    attempts.append({"select": "*", "theme_name": f"eq.{theme_name}", "limit": str(limit)})
    ...
    for params in attempts:
        ...
        if response.status_code < 400:
            ...
            if "run_date_sgt" in df.columns:
                if start_date:
                    df = df[df["run_date_sgt"] >= pd.to_datetime(start_date)]
                if end_date:
                    df = df[df["run_date_sgt"] <= pd.to_datetime(end_date)]
                df = df.sort_values("run_date_sgt")
            return df
        last_error = response.text[:300]

    st.warning(f"Could not load {table_name}: {last_error}")
    return pd.DataFrame()
...
def plot_phase2d_regime_timeline(df: pd.DataFrame):
    ...
    if df.empty or "run_date_sgt" not in df.columns:
        _empty_chart(title)
        return
    regime_col = _first_existing_col(df, ["regime", "structural_regime", "propagation_regime", "state", "regime_label"])
...
def render_phase2d1_historical_analytics_layer(selected_theme: str = "ai") -> None:
    ...
    with tabs[0]:
        ...
        a, b = st.columns(2)
        with a:
            plot_phase2d_line(momentum_df, "Rolling Momentum Trend", ["rolling_momentum", "propagation_momentum", "momentum_score", "structural_momentum_score"], entity_cols)
        with b:
            plot_phase2d_line(momentum_df, "Propagation Acceleration", ["acceleration_score", "momentum_acceleration", "acceleration"], entity_cols, zero_line=True)
        c, d = st.columns(2)
        with c:
            plot_phase2d_bar_latest(momentum_df, "Entity Momentum Leaderboard", ["structural_momentum_score", "rolling_momentum", "propagation_momentum", "momentum_score"], entity_cols)
        with d:
            plot_phase2d_line(propagation_df, "Structural Momentum / Propagation Score", ["structural_momentum_score", "propagation_score", "transmission_score", "structural_score", "composite_score"], entity_cols)
    ...
    with tabs[4]:
        ...
        a, b = st.columns(2)
        with a:
            plot_phase2d_line(regime_df, "Regime Instability Monitoring", ["instability_score", "regime_instability", "transition_count", "acceleration_score"], entity_cols + pathway_cols, zero_line=True)
        with b:
            plot_phase2d_line(regime_df, "Structural State Evolution", ["state_score", "regime_score", "structural_score", "propagation_score"], entity_cols + pathway_cols)
