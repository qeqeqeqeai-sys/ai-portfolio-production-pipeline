...
def fetch_phase2d_history_table(
    ...
    Revision notes:
    - Your Phase 2D tables use run_date_sgt, not run_date.
    - Some future/legacy tables may use theme_name, theme, or no theme column.
    - If a filtered REST query returns zero rows, this function now continues to
      fallback attempts instead of immediately returning an empty DataFrame.
    - Final filtering is done locally using schema-tolerant column detection.
    """
    if not SUPABASE_URL or not SUPABASE_ANON_KEY:
        return pd.DataFrame()
    ...
    theme_value = (theme_name or "ai").strip()
    theme_values = list(dict.fromkeys([theme_value, theme_value.lower(), theme_value.upper()]))

    attempts: List[Dict[str, str]] = []

    # Preferred production schema: run_date_sgt + theme_name.
    for date_col in ["run_date_sgt", "run_date", "as_of_date", "created_at"]:
        for theme_col in ["theme_name", "theme"]:
            for tv in theme_values:
                ...
                if start_date and date_col not in ["created_at"]:
                    params[date_col] = f"gte.{start_date}"
                if end_date and date_col not in ["created_at"]:
                    params[date_col] = f"lte.{end_date}"
                attempts.append(params)

    # Fallback: no theme/date filters. Useful when PostgREST rejects a column or
    # when the table has slightly different schema. We filter locally after load.
    for date_col in ["run_date_sgt", "run_date", "as_of_date", "created_at"]:
        attempts.append({"select": "*", "order": f"{date_col}.asc", "limit": str(limit)})
    attempts.append({"select": "*", "limit": str(limit)})
    ...
    for params in attempts:
        ...
        # Local theme filtering, case-insensitive, only when a theme column exists.
        theme_col = _first_existing_col(df, ["theme_name", "theme"])
        if theme_col and theme_value:
            df = df[df[theme_col].astype(str).str.lower() == theme_value.lower()].copy()

        # Local date filtering using the normalized run_date_sgt alias.
        if "run_date_sgt" in df.columns:
            if start_date:
    ...
    # Keep this quiet unless every attempt failed with an actual error.
    if last_error and empty_successes == 0:
